...
contract HyperswapBridgeRouter is IHyperswapBridgeRouter, BridgeRouter {
    ...
    uint8 constant public CUSTODIAN_RESPONSE = 0x12;
    ...
    function initialize(address _abacusConnectionManager, address _custodian, address _router, bool _isHub) external initializer {
    ...
    function _handle(
        ...
        if (message.messageType == CUSTODIAN_MESSAGE) {
            ...
            (CustodianMessage memory cMessage) = abi.decode(message.payload, (CustodianMessage));
            bool success = IHyperswapCustodian(custodian).execute(cMessage.seqId, cMessage.opIndex, cMessage.opType, cMessage.payload);
            CustodianResponse memory cResponse = CustodianResponse(cMessage.seqId, cMessage.opIndex, success);
            Message memory response = Message(CUSTODIAN_RESPONSE, abi.encode(cResponse));
            ...
            (CustodianResponse memory cResponse) = abi.decode(message.payload, (CustodianResponse));
            IHyperswapRouter(router).handleCustodianResponse(domain, cResponse.seqId, cResponse.opIndex, cResponse.success);
        } else {
