...
library HyperswapLiquidityMathLibrary {
    ...
    function getReservesAfterArbitrage(
        ...
        (reserveA, reserveB) = HyperswapLibrary.getReserves(factory, tokenA, tokenB);

        require(reserveA > 0 && reserveB > 0, "HyperswapArbitrageLibrary: ZERO_PAIR_RESERVES");
        ...
        if (aToB) {
            uint amountOut = HyperswapLibrary.getAmountOut(amountIn, reserveA, reserveB);
            reserveA += amountIn;
            reserveB -= amountOut;
        } else {
            uint amountOut = HyperswapLibrary.getAmountOut(amountIn, reserveB, reserveA);
            reserveB += amountIn;
            reserveA -= amountOut;
        }
    }
    ...
    function getLiquidityValue(
        ...
        IHyperswapPair pair = IHyperswapPair(HyperswapLibrary.pairFor(factory, tokenA, tokenB));
        bool feeOn = IHyperswapFactory(factory).feeTo() != address(0);
