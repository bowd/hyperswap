...
library HyperswapLiquidityMathLibrary {
  ...
  function getReservesAfterArbitrage(
    ...
    (reserveA, reserveB) = HyperswapLibrary.getReserves(
      factory,
      tokenA,
      tokenB
    );
    ...
    require(
      ...
      "HyperswapArbitrageLibrary: ZERO_PAIR_RESERVES"
    );
    ...
    if (aToB) {
      uint256 amountOut = HyperswapLibrary.getAmountOut(
        amountIn,
        reserveA,
        reserveB
      );
  ...
  function getLiquidityValue(
    ...
    (uint256 reservesA, uint256 reservesB) = HyperswapLibrary.getReserves(
      factory,
      tokenA,
      tokenB
    );
    IHyperswapPair pair = IHyperswapPair(
      HyperswapLibrary.pairFor(factory, tokenA, tokenB)
    );
