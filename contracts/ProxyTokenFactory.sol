...
contract ProxyTokenFactory is IProxyTokenFactory {
    function deployProxyToken(address owner, uint32 domain, address remoteToken, address pair) external returns (address) {
        AccountingERC20 accountingToken = new AccountingERC20(domain, remoteToken, pair);
