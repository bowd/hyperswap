...
contract HyperswapPair is IHyperswapPair, HyperswapERC20 {
    ...
    function getReserves() public view returns (uint112 _reserveLocal, uint112 _reserveRemote, uint32 _blockTimestampLast) {
    ...
    function initialize(Token calldata _tokenLocal, Token calldata _tokenRemote, address _tokenRemoteProxy) external {
    ...
    function _update(uint256 balanceLocal, uint256 balanceRemote, uint112 _reserveLocal, uint112 _reserveRemote) private {
        ...
        if (timeElapsed > 0 && _reserveLocal != 0 && _reserveRemote != 0) {
            ...
            priceLocalCumulativeLast += uint(UQ112x112.encode(_reserveRemote).uqdiv(_reserveLocal)) * timeElapsed;
            priceRemoteCumulativeLast += uint(UQ112x112.encode(_reserveLocal).uqdiv(_reserveRemote)) * timeElapsed;
        }
    ...
    function _mintFee(uint112 _reserveLocal, uint112 _reserveRemote) private returns (bool feeOn) {
        address feeTo = IHyperswapFactory(factory).feeTo();
        feeOn = feeTo != address(0);
        uint256 _kLast = kLast; // gas savings
        if (feeOn) {
            if (_kLast != 0) {
                uint256 rootK = Math.sqrt(uint(_reserveLocal) * _reserveRemote);
    ...
    function mint(address to) external lock returns (uint256 liquidity) {
        ...
        uint256 balanceLocal = IERC20(tokenLocal.tokenAddr).balanceOf(address(this));
        ...
        if (_totalSupply == 0) {
           ...
           _mint(address(0), MINIMUM_LIQUIDITY); // permanently lock the first MINIMUM_LIQUIDITY tokens
            ...
            liquidity = Math.min((amount0 * _totalSupply) / _reserveLocal, (amount1 * _totalSupply) / _reserveRemote);
        }
    ...
    function swap(uint256 amount0Out, uint256 amount1Out, address to, bytes calldata data) external lock {
        ...
        (uint112 _reserveLocal, uint112 _reserveRemote,) = getReserves(); // gas savings
        require(amount0Out < _reserveLocal && amount1Out < _reserveRemote, "Hyperswap: INSUFFICIENT_LIQUIDITY");
        ...
        uint256 amount0In = balanceLocal > _reserveLocal - amount0Out ? balanceLocal - (_reserveLocal - amount0Out) : 0;
        uint256 amount1In = balanceRemote > _reserveRemote - amount1Out ? balanceRemote - (_reserveRemote - amount1Out) : 0;
        ...
        require(balanceLocalAdjusted * balanceRemoteAdjusted >= uint(_reserveLocal) * _reserveRemote * 1000**2, "Hyperswap: K");
        }
    ...
    function skim(address to) external lock {
        ...
        _safeTransfer(_tokenLocal, to, IERC20(_tokenLocal).balanceOf(address(this)) - reserveLocal);
        _safeTransfer(_tokenRemote, to, IERC20(_tokenRemote).balanceOf(address(this)) - reserveRemote);
    }
